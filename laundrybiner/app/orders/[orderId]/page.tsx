...
import { Calendar, Clock, MapPin, Phone, User, Package, CheckCircle, Truck, ArrowLeft, Receipt, CreditCard, Timer, Star } from "lucide-react"
...
export default function OrderDetailPage() {  
  ...
  useEffect(() => {
    // Always fetch order detail, regardless of auth status for demo
    if (params.orderId) {
  ...
  const fetchOrderDetail = async (orderId: string) => {
    ...
    try {
      ...
      if (!session?.access_token) {
        setError('Tidak dapat mengakses data pesanan. Silakan login kembali.')
        return
      }
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          if (data.data.status === 'completed' || data.data.feedback_id) {
        ...
        setError('Pesanan tidak ditemukan atau Anda tidak memiliki akses ke pesanan ini.')
      } else {        const errorData = await response.json().catch(() => ({}));
        console.error('Failed to fetch order detail:', response.status, errorData)
        setError(errorData.message || 'Gagal memuat detail pesanan')
      }
    } catch (error) {
  ...
  const fetchFeedback = async (orderId: string) => {
    try {
      const response = await fetch(`/api/orders/feedback?orderId=${orderId}`)
      if (response.ok) {
        const data = await response.json()
  ...
  const handleSubmitFeedback = async (feedbackData: any) => {
    ...
    try {
      const response = await fetch('/api/orders/feedback', {
        ...
        body: JSON.stringify({
          orderId: order.id,
          userId: null, // For demo, we don't have auth user
          ...feedbackData,
        }),
      })
      ...
      if (data.success) {
        ...
        await fetchOrderDetail(order.id)
        alert('Terima kasih atas feedback Anda! Pesanan telah selesai.')
      } else {
  ...
  const getOrderSteps = () => {
    ...
    steps.push({ 
      ...
      label: 'Selesai', 
      icon: Star, 
      description: 'Pesanan telah selesai dengan feedback' 
    })
  ...
  }  // Show loading when fetching order detail (not dependent on auth in demo mode)
  if (loading) {
    return (
                ...
                <Card className="sticky top-4 border-gray-200 shadow-sm">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="text-xs text-gray-500 pt-4 border-t">
                      ...
                      {/* Finish Order Button - Show only if order is delivered and not completed */}
                      {order.status === 'delivered' && !feedback && (
                        ...
                        >
                          ...
                          {submittingFeedback ? 'Memproses...' : 'Selesaikan Pesanan'}
                        </Button>
                      )}

                      {/* Show feedback info if order is completed */}
                      {order.status === 'completed' && feedback && (
                        <div className="p-3 bg-purple-50 rounded-lg border border-purple-200">
                          <div className="flex items-center gap-2 mb-2">
                            <Star className="h-4 w-4 text-purple-600" />
                            <span className="text-sm font-medium text-purple-800">Pesanan Selesai</span>
                          </div>
                          <div className="text-xs text-purple-700">
                            ...
                            <p className="mt-1 text-xs text-purple-600">
                              Diselesaikan: {new Date(feedback.created_at).toLocaleDateString('id-ID')}
                            </p>
                          </div>
                        </div>
                      )}
